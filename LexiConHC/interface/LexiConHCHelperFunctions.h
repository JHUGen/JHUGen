#ifndef LEXICONHC_HELPERFUNCTIONS_H
#define LEXICONHC_HELPERFUNCTIONS_H
...
namespace LexiConHCHelperFunctions{
...
template<typename T> void LexiConHCHelperFunctions::castStringToValue(std::string const& name, T& val){ std::stringstream ss(name); ss >> val; }

template<typename T> bool LexiConHCHelperFunctions::checkListVariable(std::vector<T> const& list, T const& var){ return (std::find(std::begin(list), std::end(list), var)!=std::end(list)); }
template<typename T> bool LexiConHCHelperFunctions::hasCommonElements(std::vector<T> const& list1, std::vector<T> const& list2){
  for (T const& el1:list1){ if (checkListVariable(list2, el1)) return true; }
  return false;
}

template<typename T, typename U> bool LexiConHCHelperFunctions::getUnorderedMapIterator(T const& key, std::unordered_map<T, U> const& theMap, typename std::unordered_map<T, U>::const_iterator& it){
  it = theMap.find(key);
  return (it!=theMap.cend());
}
template<typename T, typename U> bool LexiConHCHelperFunctions::getUnorderedMapIterator(T const& key, std::unordered_map<T, U>& theMap, typename std::unordered_map<T, U>::const_iterator& it){
  it = theMap.find(key);
  return (it!=theMap.cend());
}
template<typename T, typename U> bool LexiConHCHelperFunctions::getUnorderedMapIterator(T const& key, std::unordered_map<T, U>& theMap, typename std::unordered_map<T, U>::iterator& it){
  it = theMap.find(key);
  return (it!=theMap.end());
}

template<typename T, typename U> bool LexiConHCHelperFunctions::hasElement(std::unordered_map<T, U> const& theMap, T const& key){
  typename std::unordered_map<T, U>::const_iterator it_dummy;
  return getUnorderedMapIterator(key, theMap, it_dummy);
}
template<typename T> bool LexiConHCHelperFunctions::hasElement(std::vector<T> const& list, T const& var){ return checkListVariable(list, var); }

template<typename T, typename U> void LexiConHCHelperFunctions::getValueWithDefault(std::unordered_map<T, U> const& theMap, T const& key, U& val, U const& defval){
