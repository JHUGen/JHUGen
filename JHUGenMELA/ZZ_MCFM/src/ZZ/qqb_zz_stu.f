      subroutine qqb_zz_stu(p,msq,channeltoggle)
      implicit none
...
c     channeltoggle allows for splitting into s/t+u/s+t+u channels
c     0 = s+t+u
c     1 = s
c     2 = t+u
...
c      write(6,*) 'initial qqb=',qqb

c      write(6,*) 'channeltoggle=',channeltoggle
...
c-- quark, lepton 4 and lepton 6 respectively
      ...
      if((channeltoggle.eq.0).or.(channeltoggle.eq.2)) then
...
c       write(6,*) 'channeltoggle 0 2 qqb=',qqb(1,1,1)

      endif

      if((channeltoggle.eq.0).or.(channeltoggle.eq.1)) then
       if (srdiags) then
...
c        write(6,*) 'channeltoggle 0 1 qqb1=',qqb1(1,1,1)
...
c      write(6,*) 'channeltoggle 0 1 2 q_qb=',q_qb

      
      if (interference .eqv. .false.) then
c--- normal case
        msq(j,k)=msq(j,k)+ave*abs(q_qb)**2
c        write(6,*) 'No interf. plus q_qb 0 1 2 msq(j,k)=',msq(j,k)
