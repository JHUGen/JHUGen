module ModVVHOffshell
   ...
   real(8), parameter    :: cw = 1d0-xw
   complex(8), parameter :: gZ = ci*dsqrt(pi*alpha_QED/(cw*xw))
   complex(8), parameter :: gW = gZ*sqrt2*sqrt(cw)
   real(8), parameter :: Q(-5,5) = (/ &
                                      +0.333333333333333d0, -0.666666666666667d0, +0.333333333333333d0, -0.666666666666667d0, +0.333333333333333d0, &
                                      +0d0,                                                                                                         &
                                      -0.333333333333333d0, +0.666666666666667d0, -0.333333333333333d0, +0.666666666666667d0, -0.333333333333333d0 /)
   real(8), parameter :: tau = (/ +1d0,-1d0,+1d0,-1d0,+1d0,0d0,-1d0,+1d0,-1d0,+1d0,-1d0 /)
   ...
   subroutine qq_ZZqq(p,msq)
      implicit none
!--- Author: R.K. Ellis, October 2014
!--- q(-p1)+q(-p2)->Z(p3,p4)+Z(p5,p6)+q(p7)+q(p8);
      include 'constants.f'
      include 'cmplxmass.f'
      include 'ewcouple.f'
      include 'ewcharge.f'
      ...
      integer, parameter :: jmax=12,nmax=10
      integer :: j,k,ll
      integer, parameter :: uqcq_uqcq=1,uquq_uquq=2,dqsq_dqsq=3, &
                            dqdq_dqdq=4,uqbq_uqbq=5,dqcq_dqcq=6, &
                            dquq_dquq=7,dqcq_uqsq=8,uqsq_dqcq=9
      integer     :: h1,h2,h3,h5
      real(8)     :: p(1:4,16),msq(-5:5,-5:5),temp(-5:5,-5:5),tempw(fn:nf,fn:nf),mult
      complex(8)  :: zab(mxpart,4,mxpart),zba(mxpart,4,mxpart),cdotpr,                 &
      ...
      logical     :: doHO,doBO
      complex(8)  :: Hbit,Bbit
      real(8), parameter :: spinavge=0.25d0,stat=0.5d0
...
! MARKUS: choosing ZZ final state
      ...
      doHO=.true.
      mult=1d0
!--- rescaling factor for Higgs amplitudes, if anomalous Higgs width
!      if (anom_Higgs) then
!         mult=chi_higgs**2
!      endif
...
!---setup spinors and spinorvector products
...
!--- propagators and currents are not used in calculation of Higgs contribution
...
!-----Singly resonant production in VBF style diagrams
         ...
         endif ! doHD .eqv. .false.


!----ZZ->ZZ scattering with the exchange of a H
         call ZZHZZamp(j1(j),j2(j),3,4,5,6,j7(j),j8(j),
        & za,zb,ZZHamp71_82)
         call ZZHZZamp(j1(j),j2(j),3,4,5,6,j8(j),j7(j),
        & za,zb,ZZHamp81_72)
!----Four boson vertex + WW->Higgs diagram 
...
!-----setup for uqsq_dqcq W diagrams (2,3)->(1,4)
         do h1=1,1
         do h2=1,1
         do h3=1,2
         do h5=1,2
...
!--------------------------------------------------------
!-----setup for dquq_dquq W diagrams (1,2)-->(1,2)
         do h1=1,2
         do h2=1,2
         do h3=1,2
         do h5=1,2
