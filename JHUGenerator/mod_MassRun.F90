...
SUBROUTINE Kinematics_VH(helicity,id,MomExt,NBin,applyPSCut,HDecays,PhoOnshell)
use ModMisc
use ModParameters
implicit none

real(8), intent(in) :: helicity(1:9)
...
complex(8) :: amp_gg,amp_lo
real(8) :: Dgg2qq, me2gg, me2lo,mass(3:5,1:2)
...
! Dqq2gg
        if(enable_gg2qq.eqv..true.)then
            ...
            call amp_VH_LO(MomExt(:,1:9),mass(3:5,1:2),helicity(1:9),id(1:9),amp_lo)
            call amp_VH_gg(MomExt(:,1:9),mass(3:5,1:2),helicity(1:9),id(1:9),amp_gg)
            me2lo=dble(amp_lo*dconjg(amp_lo))
            me2gg=dble(amp_gg*dconjg(amp_gg))
            Dgg2qq=me2gg/(me2gg+me2lo)
        else
            Dgg2qq=1d0
        endif
