...
do i4 = 1,2
         ...
         call calcHelAmp_gg(ordering,VVMode,p(1:4,1:6),MY_IDUP,i1,i2,i3,i4,A(1))
         if( includeInterference .and. (MY_IDUP(6).eq.MY_IDUP(8)) .and. (MY_IDUP(7).eq.MY_IDUP(9)) ) then
             call calcHelAmp_gg(ordering_swap,VVMode,p(1:4,1:6),MY_IDUP,i1,i2,i3,i4,A(2))
...
enddo
      ...
      call EvalAmp_gg_G_VV_old(p,MY_IDUP,res2)
      print *,"EvalAmp_gg_G_VV::"
...
do i4 = 1,2
         ...
         call calcHelAmp_qq(ordering,VVMode,p(1:4,1:6),MY_IDUP,i1,i3,i4,A(1))
         if( includeInterference .and. (MY_IDUP(6).eq.MY_IDUP(8)) .and. (MY_IDUP(7).eq.MY_IDUP(9)) ) then
             call calcHelAmp_qq(ordering_swap,VVMode,p(1:4,1:6),MY_IDUP,i1,i3,i4,A(2))
             A(2) = -A(2) ! minus comes from fermi statistics
         endif
         if( includeInterference .and. (MY_IDUP(6).eq.MY_IDUP(8)) .and. (MY_IDUP(7).eq.MY_IDUP(9)) ) then
             sum = sum + SymmFac * (cdabs( A(1)*dconjg(A(1)) ) + cdabs( A(2)*dconjg(A(2)) ))
...
enddo
      ...
      call EvalAmp_qqb_G_VV_old(p,MY_IDUP,res2)
      print *,"EvalAmp_qqb_G_VV::"
      print *,"MY_IDUP(6)",convertLHE(MY_IDUP(6))
      print *,"MY_IDUP(7)",convertLHE(MY_IDUP(7))
      print *,"MY_IDUP(8)",convertLHE(MY_IDUP(8))
      print *,"MY_IDUP(9)",convertLHE(MY_IDUP(9))
...
!== OLD
      subroutine EvalAmp_gg_G_VV_old(p,MY_IDUP,sum)
      use ModMisc
      implicit none
      real(dp), intent(out) ::  sum
      real(dp), intent(in) :: p(4,6)
...
do i4 = 1,2
         ...
         ordering = (/3,4,5,6/)
         call calcHelAmp_old_gg(ordering,p(1:4,1:6),i1,i2,i3,i4,A(1))

         if( (includeInterference.eqv..true.) .and. (MY_IDUP(6).eq.MY_IDUP(8)) .and. (MY_IDUP(7).eq.MY_IDUP(9)) ) then
             ordering = (/5,4,3,6/)
             call calcHelAmp_old_gg(ordering,p(1:4,1:6),i1,i2,i3,i4,A(2))
             A(2) = -A(2) ! minus comes from fermi statistics
         endif
...
enddo
     ...
     subroutine EvalAmp_qqb_G_VV_old(p,MY_IDUP,sum)
      implicit none
      real(dp), intent(out) ::  sum
      real(dp), intent(in) :: p(4,6)
      integer, intent(in) :: MY_IDUP(6:9)
...
do i4 = 1,2
         ...
         ordering = (/3,4,5,6/)
         call calcHelAmp_old_qq(ordering,p(1:4,1:6),i1,i3,i4,A(1))

         if( (includeInterference.eqv..true.) .and. (MY_IDUP(6).eq.MY_IDUP(8)) .and. (MY_IDUP(7).eq.MY_IDUP(9)) ) then
             ordering = (/5,4,3,6/)
             call calcHelAmp_old_qq(ordering,p(1:4,1:6),i1,i3,i4,A(2))
             A(2) = -A(2) ! minus comes from fermi statistics
         endif
...
enddo
     ...
     subroutine calcHelAmp_old_gg(ordering,p,i1,i2,i3,i4,A)
     implicit none
...
!-------- -1 == left, 1 == right
     ...
     subroutine calcHelAmp_old_qq(ordering,p,i1,i3,i4,A)
     implicit none
