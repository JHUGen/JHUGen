...
! yRnd(17): accept or reject in unweighted events
    real(8) :: yRnd(1:20),VgsWgt, EvalWeighted_VH
    real(8) :: pdf(-6:6,1:2)
...
!if pp collider
  if(Collider.eq.1.or.Collider.eq.2)then
    ...
    call EvalPhaseSpace_VH(yRnd,MomExt,inv_mass,mass,PSWgt,HbbDecays,PhoOnshell=(IsAPhoton(DecayMode1)),ZAinterference=includeGammaStar)
!    do i=1,9
!    print *, MomExt(:,i)
!    enddo
    call Kinematics_VH(id,MomExt,inv_mass,NBin,applyPSCut,HbbDecays,PhoOnshell=(IsAPhoton(DecayMode1)))
!    print *, "!!!!!!!!!"
    ...
    LO_Res_Unpol=0d0
    EvalWeighted_VH=0d0
    if((VH_PC.eq."qq".or.VH_PC.eq."lo").and.PChannel.eq.1)then
      do i = -6,6
        j = -i
        id(1:2) = (/LHA2M_PDF(i),LHA2M_PDF(j)/)
        if (abs(LHA2M_PDF(i)).ne.6   .and.   abs(LHA2M_PDF(j)).ne.6.  .and.  i.ne.0)then
          call EvalAmp_VH(id,helicity,MomExt,me2)
        else
          me2=0d0
        endif
          LO_Res_Unpol = me2*pdf(i,1)*pdf(j,2)* PreFac *PostFac
          EvalWeighted_VH = EvalWeighted_VH + LO_Res_Unpol
      enddo
    elseif((VH_PC.eq."gg".or.VH_PC.eq."bo".and.VH_PC.eq."tr").or.PChannel.eq.0)then
      if( Ehat .ge. 2d0*m_top ) return !without complex top mass in ZH amplitude
      id(1)=0
      id(2)=0
      i=0
...
elseif( IsAWDecay(DecayMode1) ) then
  ...
  call EvalPhaseSpace_VH(yRnd,MomExt,inv_mass,mass,PSWgt,HbbDecays)
  call Kinematics_VH(id,MomExt,inv_mass,NBin,applyPSCut,HbbDecays)
  if( applyPSCut .or. PSWgt.eq.zero ) return
  call SetRunningScales( (/ MomExt(1:4,5),MomExt(1:4,6),MomExt(1:4,7) /) , (/ convertLHEreverse(id(3)),convertLHEreverse(id(6)),convertLHEreverse(id(7)),convertLHEreverse(id(4)) /) )
  call setPDFs(eta1,eta2,pdf)
  ...
  do j = -5,5
     ...
     ((id2(2).eq.convertLHE(Up_).or.id2(2).eq.convertLHE(Chm_)) .and. &
      (id2(1).eq.convertLHE(ADn_) .or. id2(1).eq.convertLHE(AStr_) .or. id2(1).eq.convertLHE(ABot_)))   )then
         ...
         helicity(7)=-helicity(6)
         call EvalAmp_VH(id2,helicity,MomExt,me2)
...
!if e+ e- collider
      ...
      if( IsNaN(EvalWeighted_VH) ) then
         EvalWeighted_VH = 0d0
         return
      endif
      if(EvalWeighted_VH.ne.0d0)then
         call WriteOutEvent_VH(id,helicity,MomExt,inv_mass,EventWeight=EvalWeighted_VH*VgsWgt)
      endif
   endif

   do NHisto = 1,NumHistograms
      call intoHisto(NHisto,NBin(NHisto),EvalWeighted_VH*VgsWgt)
   enddo
...
#endif
implicit none
real(8) :: yRnd(1:21), EvalUnWeighted_VH, RES(-5:5,-5:5)
real(8) :: pdf(-6:6,1:2)
real(8) :: eta1, eta2, FluxFac, Ehat, sHatJacobi
real(8) :: MomExt(1:4,1:9), PSWgt, PSWgt2
...
!if pp collider
  if(Collider.eq.1.or.Collider.eq.2)then
    ...
    EvalUnWeighted_VH = LO_Res_Unpol
!if e+ e- collider
  else if(Collider.eq.0)then
    ifound=0
    jfound=0
...
!pp>WH
  ...
  if( EvalUnWeighted_VH.gt. CS_max) then
    ...
    Res = 0d0
  elseif( EvalUnWeighted_VH .gt. yRnd(17)*CS_max ) then
    call Kinematics_VH(id,MomExt,inv_mass,NBin,applyPSCut,HbbDecays,PhoOnshell=(IsAPhoton(DecayMode1)))
    do NHisto=1,NumHistograms
    ...
    do i=6,7
      inv_mass(i)=dsqrt(dabs(MomExt(1:4,i).dot.MomExt(1:4,i)))
    enddo
    call WriteOutEvent_VH(id,helicity,MomExt,inv_mass,EventWeight=1d0)
  else
  ...
  if(Collider.eq.1.or.Collider.eq.2)then
    if((VH_PC.eq."qq".or.VH_PC.eq."lo").and.PChannel.eq.1)then
      do i = -5,5
        ...
        LO_Res_Unpol = me2 *pdf(LHA2M_PDF(i),1)*pdf(LHA2M_PDF(j),2) * PreFac *PostFac
        EvalUnWeighted_VH = EvalUnWeighted_VH+LO_Res_Unpol
        RES(i,j) = LO_Res_Unpol
        if (LO_Res_Unpol.gt.csmax(i,j)) then
          csmax(i,j) = LO_Res_Unpol
        endif
      enddo
    elseif((VH_PC.eq."gg".or.VH_PC.eq."bo".or.VH_PC.eq."tr").and.PChannel.eq.0)then
      me2=0d0
      if( Ehat .ge. 2d0*m_top ) return !without complex top mass in ZH amplitude
      id(1)=0
      id(2)=0
