...
!        call boost_from_CMS_to_RefMom(p_boost,Mom2(:))
  ...
  real(8), optional :: PropPower
      ...
      if (sMin .gt. sMax) then
         InvMass_sq = 0d0
         s_channel_propagator = 0d0
      elseif( Width.lt.1d-15 ) then
...
! ----- auxilary functions ---------
  ...
  FUNCTION h(RandomVar,m_sq_in,nu,smin,smax)! inv. mass mapping for s- and t-channel propagator
  implicit none
  real(8) :: h,RandomVar,m_sq_in,m_sq,nu,smin,smax
...
! this is -h(RandomVar,-m_sq,nu,-smin,-smax), i.e. h(..) for t-channel
  ...
  FUNCTION k_BreitWigner(RandomVar,m_sq,gamma,smin,smax,InvMass_sq)
  implicit none
  real(8) :: k_BreitWigner,RandomVar,m_sq,gamma,smin,smax,m,y1,y2,InvMass_sq
  ...
  real(8), parameter :: rthr = 0.99999d0
    ...
    y = tan(pi*(min(max(r,-rthr),rthr) - 0.5d0)) ! Unitless at the moment
