...
subroutine amp_VH_LO(MomExt,mass,helicity,id,amp)
...
! amplitudes w/o HVV or VFF couplings to calculate
  if(id(6).ne.convertLHE(Pho_))then !Z/W/A decays
    call spinoru2(4,(/MomExt(1:4,1),MomExt(1:4,2),MomExt(1:4,6),MomExt(1:4,7)/),Spaa,Spbb,sprod)
    ...
    call spinoru2(4,(/MomExt(1:4,1),MomExt(1:4,2),MomExt(1:4,2),MomExt(1:4,4)/),Spaa,Spbb,sprod)!reference momentum to be set!!!!!!!!!!!!!!!
  ...
  if(id(1).gt.0d0)then!q q~
    ...
    if(id(1)*helicity(1).lt.0d0)then!J1 is left
      if(id(6)*helicity(6).lt.0d0)then!J2 is left
        ...
        qqffbHa1 = - 2d0*Spaa(1,4)*Spbb(2,3)!LR
      endif
    else !J1 is right
      if(id(6)*helicity(6).lt.0d0)then!J2 is left
        qqffbHa1 = - 2d0*Spaa(2,3)*Spbb(1,4)!RL
      else!J2 is right
    ...
    if(id(1)*helicity(1).lt.0d0)then!J1 is left
      if(id(6)*helicity(6).lt.0d0)then!J2 is left
        qqffbHa2 = - Spaa(1,3)**2*Spbb(1,4)*Spbb(2,3) &
                   - Spaa(1,3)*Spaa(1,4)*Spbb(1,4)*Spbb(2,4) &
                   - Spaa(1,3)*Spaa(2,3)*Spbb(2,3)*Spbb(2,4) &
                   - Spaa(1,4)*Spaa(2,3)*Spbb(2,4)**2!LL
      else!J2 is right
        qqffbHa2 = - Spaa(1,3)*Spaa(1,4)*Spbb(1,3)*Spbb(2,3) &
                   - Spaa(1,3)*Spaa(2,4)*Spbb(2,3)**2 &
                   - Spaa(1,4)**2*Spbb(1,3)*Spbb(2,4) &
                   - Spaa(1,4)*Spaa(2,4)*Spbb(2,3)*Spbb(2,4)!LR
      endif
    else!J1 is right
      if(id(6)*helicity(6).lt.0d0)then!J2 is left
        qqffbHa2 = - Spaa(1,3)*Spaa(2,3)*Spbb(1,3)*Spbb(1,4) &
                   - Spaa(1,3)*Spaa(2,4)*Spbb(1,4)**2 &
                   - Spaa(2,3)**2*Spbb(1,3)*Spbb(2,4) &
                   - Spaa(2,3)*Spaa(2,4)*Spbb(1,4)*Spbb(2,4)!RL
      else!J2 is right
        qqffbHa2 = - Spaa(1,4)*Spaa(2,3)*Spbb(1,3)**2 &
                   - Spaa(1,4)*Spaa(2,4)*Spbb(1,3)*Spbb(1,4) &
                   - Spaa(2,3)*Spaa(2,4)*Spbb(1,3)*Spbb(2,3) &
                   - Spaa(2,4)**2*Spbb(1,4)*Spbb(2,3)!RR
      endif
    endif
  endif
...
subroutine qqbffbHg4(Spaa,Spbb,sprod,id,helicity,qqffbHg4)
  ...
  if(id(1).gt.0d0)then!q q~
    ...
    if(id(1)*helicity(1).lt.0d0)then!J1 is left
      if(id(6)*helicity(6).lt.0d0)then!J2 is left
        qqffbHg4 = - Spaa(1,3)**2*Spbb(1,3)*Spbb(2,4) &
                   + Spaa(1,3)**2*Spbb(1,4)*Spbb(2,3) &
                   + Spaa(1,3)*Spaa(2,4)*Spbb(2,4)**2 &
                   - Spaa(1,4)*Spaa(2,3)*Spbb(2,4)**2!LL
      else!J2 is right
        qqffbHg4 = - Spaa(1,3)*Spaa(2,4)*Spbb(2,3)**2 &
                   + Spaa(1,4)**2*Spbb(1,3)*Spbb(2,4) &
                   - Spaa(1,4)**2*Spbb(1,4)*Spbb(2,3) &
                   + Spaa(1,4)*Spaa(2,3)*Spbb(2,3)**2!LR
      endif
    else!J1 is right
      if(id(6)*helicity(6).lt.0d0)then!J2 is left
        qqffbHg4 = - Spaa(1,2)*Spaa(3,4)*Spbb(1,4)**2 &
                   + Spaa(2,3)**2*Spbb(1,2)*Spbb(3,4)!RL
      else!J2 is right
        qqffbHg4 =   Spaa(1,3)*Spaa(2,4)*Spbb(1,3)**2 &
                   - Spaa(1,4)*Spaa(2,3)*Spbb(1,3)**2 &
                   - Spaa(2,4)**2*Spbb(1,3)*Spbb(2,4) &
...
!    if(id(1)*helicity(1).lt.0d0)then!J1 is left
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHa1 = - 2d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(2,3)*Spbb(1,4)!LL
!      else!J2 is right
!        qqAHa1 = - 2d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(2,4)*Spbb(1,3)!LR
!      endif
!    else!J1 is right
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHa1 = - 2d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spbb(2,4)!RL
!      else!J2 is right
!        qqAHa1 = - 2d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)*Spbb(2,3)!RR
!      endif
!    endif
!  else!q~ q
!    if(id(1)*helicity(1).lt.0d0)then!J1 is left
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHa1 = - 2d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spbb(2,4)!LL
!      else!J2 is right
!        qqAHa1 = - 2d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)*Spbb(2,3)!LR
!      endif
!    else !J1 is right
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHa1 = - 2d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(2,3)*Spbb(1,4)!RL
!      else!J2 is right
!        qqAHa1 = - 2d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(2,4)*Spbb(1,3)!RR
!      endif
!    endif
!  endif
...
!    if(id(1)*helicity(1).lt.0d0)then!J1 is left
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHa2 = - 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spaa(2,4)*Spbb(1,4)**2 - &
!     & 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(2,3)*Spaa(2,4)*Spbb(1,4)*Spbb(2,4)!LL
!      else!J2 is right
!        qqAHa2 = - 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)*Spaa(2,4)*Spbb(1,3)*Spbb(1&
!     & ,4) - 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(2,4)**2*Spbb(1,4)*Spbb(2,3)!LR
!      endif
!    else!J1 is right
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHa2 = - 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spaa(1,4)*Spbb(1,4)*Spbb(2&
!     & ,4) - 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,4)*Spaa(2,3)*Spbb(2,4)**2!RL
!      else!J2 is right
!        qqAHa2 = - 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)**2*Spbb(1,3)*Spbb(2,4) - &
!     & 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)*Spaa(2,4)*Spbb(2,3)*Spbb(2,4)!RR
!      endif
!    endif
!  else!q~ q
!    if(id(1)*helicity(1).lt.0d0)then!J1 is left
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHa2 = - 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spaa(1,4)*Spbb(1,4)*Spbb(2&
!     & ,4) - 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,4)*Spaa(2,3)*Spbb(2,4)**2!LL
!      else!J2 is right
!        qqAHa2 = - 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)**2*Spbb(1,3)*Spbb(2,4) - &
!     & 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)*Spaa(2,4)*Spbb(2,3)*Spbb(2,4)!LR
!      endif
!    else !J1 is right
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHa2 = - 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spaa(2,4)*Spbb(1,4)**2 - &
!     & 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(2,3)*Spaa(2,4)*Spbb(1,4)*Spbb(2,4)!RL
!      else!J2 is right
!        qqAHa2 = - 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)*Spaa(2,4)*Spbb(1,3)*Spbb(1&
!     & ,4) - 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(2,4)**2*Spbb(1,4)*Spbb(2,3)!RR
!      endif
!    endif
!  endif
...
!    if(id(1)*helicity(1).lt.0d0)then!J1 is left
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHg4 = - 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spaa(2,4)*Spbb(1,4)**2 + 1d0/(&
!     & dsqrt(2d0))/(Spbb(3,4))*Spaa(1,4)*Spaa(2,3)*Spbb(1,4)**2!LL
!      else!J2 is right
!        qqAHg4 = - 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(2,4)**2*Spbb(1,3)*Spbb(2,4) + &
!     & 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(2,4)**2*Spbb(1,4)*Spbb(2,3)!LR
!      endif
!    else!J1 is right
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHg4 = 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spaa(2,4)*Spbb(2,4)**2 - 1d0/(&
!     & dsqrt(2d0))/(Spbb(3,4))*Spaa(1,4)*Spaa(2,3)*Spbb(2,4)**2!RL
!      else!J2 is right
!        qqAHg4 = 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)**2*Spbb(1,2)*Spbb(3,4)!RR
!      endif
!    endif
!  else!q~ q
!    if(id(1)*helicity(1).lt.0d0)then!J1 is left
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHg4 = 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spaa(2,4)*Spbb(2,4)**2 - 1d0/(&
!     & dsqrt(2d0))/(Spbb(3,4))*Spaa(1,4)*Spaa(2,3)*Spbb(2,4)**2!LL
!      else!J2 is right
!        qqAHg4 = 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)**2*Spbb(1,3)*Spbb(2,4) - 1d0/(&
!     & dsqrt(2d0))/(Spaa(3,4))*Spaa(1,4)**2*Spbb(1,4)*Spbb(2,3)!LR
!      endif
!    else !J1 is right
!      if(id(6)*helicity(6).lt.0d0)then!J2 is left
!        qqAHg4 = - 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,3)*Spaa(2,4)*Spbb(1,4)**2 + &
!     & 1d0/(dsqrt(2d0))/(Spbb(3,4))*Spaa(1,4)*Spaa(2,3)*Spbb(1,4)**2!RL
!      else!J2 is right
!        qqAHg4 = - 1d0/(dsqrt(2d0))/(Spaa(3,4))*Spaa(2,4)**2*Spbb(1,2)*Spbb(3,4)!RR
!      endif
!    endif
!  endif
