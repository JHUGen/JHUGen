...
! 555555555 Higgs
  if(HDecays)then
    Jac3 = s_channel_propagator(M_Reso**2,Ga_Reso,(2d0*getMass(convertLHEreverse(id(8))))**2,Energy**2,xRnd(13),s89) ! H --> 89
  else
...
! 444444444 V on-shell
  if(hasAonshell) then
    ...
    Jac2 = s_channel_propagator(M_V**2,Ga_V,(2d0*getMass(convertLHEreverse(id(6))))**2,(Energy-dsqrt(s89))**2,xRnd(12),s67) ! V --> 67
  endif
...
SUBROUTINE EvalPhasespace_VHglu(xRnd,Energy,Mom,id,Jac,HDecays,PhoOnshell)
  ...
  Mom(:,1) = 0.5d0*Energy * (/+1d0,0d0,0d0,+1d0/) ! beam 1
  Mom(:,2) = 0.5d0*Energy * (/+1d0,0d0,0d0,-1d0/) ! beam 2
  Momab = Energy * (/1d0,0d0,0d0,0d0/) ! V*

! masses
! p3 = p1 + p2 - p10 = p6 + p7 + p8 + p9
  Jac1 = s_channel_propagator(M_V**2,Ga_Reso,(M_Reso+2d0*getMass(convertLHEreverse(id(6))))**2,Energy**2,xRnd(14),s6789) ! 1+2 ---> 10+V*(6789)

! 555555555 Higgs
  if(HDecays)then
    Jac3 = s_channel_propagator(M_Reso**2,Ga_Reso,(2d0*getMass(convertLHEreverse(id(8))))**2,s6789,xRnd(13),s89) ! H --> 89
  else
    s89 = M_Reso**2 ! if H onshell
    Jac3 = 1d0
  endif
...
!        SingDepth = 1e-13
!        Steps = 10
!        call gensing(4,Energy,(/M_Reso,0d0,0d0,0d0/),(/Mom(:,9),Mom(:,6),Mom(:,7),Mom(:,10)/),Pcol1,Pcol2,SingDepth,Steps)
!        Mom(1:4,8) = Mom(1:4,9)
!        Mom(1:4,9) = 0d0
!        Jac=1d0
