...
!#if linkMELA==1
subroutine amp_VH_gg(Mom,mass,helicity,id,amp)
  implicit none
...
!LO branching fraction of Z > e- e+
!print*,alpha_QED/12d0*m_Z*((aR_lep+aL_lep)**2+(aR_lep-aL_lep)**2)/(2d0*sitW*dsqrt(1d0-xw))**2/Ga_Z!!!!!!!!!!!!!!!!!!!!!!!

!print *, gVVS1(1)/vev
!print *, dsqrt(4d0*pi*alpha_QED) /sitW/M_W *M_Z**2
...
!gVVS2=0d0
!gVVS2(1)=ci*dsqrt(4d0*pi*alpha_QED) /sitW/M_W  *M_Z**2* ((8d0,5d0)/M_reso**2)
...
! amplitudes w/o HVV or VFF couplings to calculate
  if(id(6).ne.convertLHE(Pho_))then !Z/A decays
    ...
    if(VH_PC.eq."bo".or.VH_PC.eq."gg")then
    ...
    if(VH_PC.eq."bo")then!gg > box couples to axial vector only, which photon is not.
