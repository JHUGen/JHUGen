...
!#if linkMELA==1
subroutine amp_VH_gg(MomExt,mass,helicity,id,amp)
  implicit none
...
gVVS1=0d0
  gVVS1(1) = ci*dsqrt(4d0*pi*alpha_QED) /sitW/M_W  *M_Z**2 *(10d0,7d0)
!  gVVS1(1) = ci*dsqrt(4d0*pi*alpha_QED) /sitW/M_W  *M_Z**2
!  print*,"gVVS1(1) = ",gVVS1(1)
!  gVVS1(1) = 0d0

gVVS2=0d0
  gVVS2(1)=ci*dsqrt(4d0*pi*alpha_QED) /sitW/M_W  * ((8d0,5d0)*M_Z**2/M_reso**2)
  !gVVS2(1)=ci*dsqrt(4d0*pi*alpha_QED) /sitW/M_W  * ((8d0,5d0)/M_reso**2)

!gVVS1=0d0
!gVVS1(1)=ci*dsqrt(4d0*pi*alpha_QED) /sitW/M_W  *M_Z**2
!print*,"gVVS2(1) = ", dsqrt(4d0*pi*alpha_QED) /sitW/M_W 
!print*,"q3_q3,q4_q4,q5_q5 = ",Get_MInv(MomExt(:,3)),Get_MInv(MomExt(:,4)),Get_MInv(MomExt(:,5))
...
! amplitudes w/o HVV or VFF couplings to calculate
  if(id(6).ne.convertLHE(Pho_))then !Z/A decays
    call spinoru2(4,(/MomExt(1:4,1),MomExt(1:4,2),MomExt(1:4,6),MomExt(1:4,7)/),Spaa,Spbb,sprod) !!!correct one
    !call spinoru2(4,(/-MomExt(1:4,1),-MomExt(1:4,2),MomExt(1:4,7),MomExt(1:4,6)/),Spaa,Spbb,sprod) !!!test only
    ...
    if(VHiggs_PC.eq."bo".or.VHiggs_PC.eq."gg")then
      call ggBoxffbH(MomExt,Spaa,Spbb,sprod,helicity,BoxffbH)!SM
      if(kappa_tilde.ne.(0d0,0d0))then
        call ggBoxffbH5(MomExt,Spaa,Spbb,sprod,helicity,BoxffbH5)!gamma5 in Hff coupling
      endif
    endif
    ...
    if(VHiggs_PC.eq."bo")then!gg > box couples to axial vector only, which photon is not.
...
! assemble everything and get iM
  if(id(8).ne.Not_a_particle_) then
    ...
    *(kappa*FFS(id(8), MomExt(:,8), helicity(8), id(9), MomExt(:,9), helicity(9)) &
      +ci*kappa_tilde*FFP(id(8), MomExt(:,8), helicity(8), id(9), MomExt(:,9), helicity(9)))&
    *(-ci/vev*getMass(convertLHEreverse(id(8))))
  endif ! else H does not decay



!print*,"cdabs(amp) =",cdabs(amp),helicity(1),helicity(2),helicity(6)
  ...
  elseif(helicity(1).gt.0d0)then
    if(helicity(6).gt.0d0)then
      Tri=-Spbb(1,2)*(m_Z**2 + Spaa(1,2)*Spbb(1,2))* &
          ...
          (2*Spaa(1,2)*Spbb(1,2) - Spaa(1,3)*Spbb(1,3) -  &
          Spaa(1,4)*Spbb(1,4) - Spaa(2,3)*Spbb(2,3) -  &
...
subroutine ggBoxffbH(MomExt,Spaa,Spbb,sprod,helicity,Box)
  implicit none
  real(8), intent(in) :: MomExt(1:4,1:9)
  complex(8), intent(in) :: Spaa(1:4,1:4),Spbb(1:4,1:4)
  real(8), intent(in) :: sprod(1:4,1:4)
  ...
  if(helicity(1).gt.0d0.and.helicity(2).lt.0d0)then
    if(helicity(6).gt.0d0)then !+-+
      call  ffbHBox7_8pmp(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbHBox9_10pmp(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbHBox11_12pmp(MomExt,Spaa,Spbb,sprod,Box11_12)
    else !+--
      call  ffbHBox7_8pmm(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbHBox9_10pmm(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbHBox11_12pmm(MomExt,Spaa,Spbb,sprod,Box11_12)
    endif
  elseif(helicity(1).lt.0d0.and.helicity(2).gt.0d0)then
    if(helicity(6).gt.0d0)then !-++
      call  ffbHBox7_8mpp(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbHBox9_10mpp(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbHBox11_12mpp(MomExt,Spaa,Spbb,sprod,Box11_12)
    else !-+-
      call  ffbHBox7_8mpm(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbHBox9_10mpm(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbHBox11_12mpm(MomExt,Spaa,Spbb,sprod,Box11_12)
    endif
  elseif(helicity(1).gt.0d0.and.helicity(2).gt.0d0)then
    if(helicity(6).gt.0d0)then !+++
      call  ffbHBox7_8ppp(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbHBox9_10ppp(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbHBox11_12ppp(MomExt,Spaa,Spbb,sprod,Box11_12)
    else !++-
      call  ffbHBox7_8ppm(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbHBox9_10ppm(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbHBox11_12ppm(MomExt,Spaa,Spbb,sprod,Box11_12)
    endif
  else
    if(helicity(6).gt.0d0)then !--+
      call  ffbHBox7_8mmp(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbHBox9_10mmp(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbHBox11_12mmp(MomExt,Spaa,Spbb,sprod,Box11_12)
    else !---
      call  ffbHBox7_8mmm(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbHBox9_10mmm(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbHBox11_12mmm(MomExt,Spaa,Spbb,sprod,Box11_12)
    endif
  endif
...
subroutine ggBoxffbH5(MomExt,Spaa,Spbb,sprod,helicity,Box)
  implicit none
  real(8), intent(in) :: MomExt(1:4,1:9)
  complex(8), intent(in) :: Spaa(1:4,1:4),Spbb(1:4,1:4)
  real(8), intent(in) :: sprod(1:4,1:4)
  ...
  if(helicity(1).gt.0d0.and.helicity(2).lt.0d0)then
    if(helicity(6).gt.0d0)then !+-+
      call  ffbH5Box7_8pmp(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbH5Box9_10pmp(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbH5Box11_12pmp(MomExt,Spaa,Spbb,sprod,Box11_12)
    else !+--
      call  ffbH5Box7_8pmm(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbH5Box9_10pmm(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbH5Box11_12pmm(MomExt,Spaa,Spbb,sprod,Box11_12)
    endif
  elseif(helicity(1).lt.0d0.and.helicity(2).gt.0d0)then
    if(helicity(6).gt.0d0)then !-++
      call  ffbH5Box7_8mpp(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbH5Box9_10mpp(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbH5Box11_12mpp(MomExt,Spaa,Spbb,sprod,Box11_12)
    else !-+-
      call  ffbH5Box7_8mpm(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbH5Box9_10mpm(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbH5Box11_12mpm(MomExt,Spaa,Spbb,sprod,Box11_12)
    endif
  elseif(helicity(1).gt.0d0.and.helicity(2).gt.0d0)then
    if(helicity(6).gt.0d0)then !+++
      call  ffbH5Box7_8ppp(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbH5Box9_10ppp(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbH5Box11_12ppp(MomExt,Spaa,Spbb,sprod,Box11_12)
    else !++-
      call  ffbH5Box7_8ppm(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbH5Box9_10ppm(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbH5Box11_12ppm(MomExt,Spaa,Spbb,sprod,Box11_12)
    endif
  else
    if(helicity(6).gt.0d0)then !--+
      call  ffbH5Box7_8mmp(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbH5Box9_10mmp(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbH5Box11_12mmp(MomExt,Spaa,Spbb,sprod,Box11_12)
    else !---
      call  ffbH5Box7_8mmm(MomExt,Spaa,Spbb,sprod,Box7_8)
      call  ffbH5Box9_10mmm(MomExt,Spaa,Spbb,sprod,Box9_10)
      call ffbH5Box11_12mmm(MomExt,Spaa,Spbb,sprod,Box11_12)
    endif
  endif
