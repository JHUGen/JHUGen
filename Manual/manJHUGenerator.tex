...
\end{verbatim}
The following example demonstrates how to modify constants used in the coupling translation, as well as the couplings that are output by JHUGeneratorLexicon. In this case we include translation from 10 independent $HVV$ and $Hgg$ couplings in the Higgs Basis to the native coefficients used in JHUGen. We return only charged current couplings including triple and quartic gauge couplings with Lambda\_w1, Lambda\_z1 and Lambda\_zgs1 set to 100GeV.
\begin{verbatim}
./JHUGenLexicon input_basis=eft_hbasis output_basis=amp_jhu dCz=1,0 Czz=.5,0 Czbx=0,1E-7 
...
\end{verbatim}
To view all of the options available, including modifiable constants, output format, and basis type, type:
\begin{verbatim}
./JHUGeneratorLexicon help
\end{verbatim}
