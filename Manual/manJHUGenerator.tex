...
We also provide a python interface for MELA, which works essentially the same way as the C\+\+ interface. It requires an instance of PyBind11, a header-only method of creating Python libraries from linked C\+\+ code, and is compiled alongside the other code.
...
>>> help(Mela)
\end{verbatim}
Since text processing is simple in Python, the Python MELA interface is especially useful for running quick jobs on LHE files. Since the Python MELA interface calls the C\+\+ code directly, it is also useful for calculations that would interface to libraries like NumPy that are commonly used in analyses.
