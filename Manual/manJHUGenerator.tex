...
After extracting the code, you can go to the directory \texttt{JHUGenMELAv2/MELA} to find code for computing matrix elements directly.
...
\textbf{Please take note: The setup relies on MCFM libraries, provided for \texttt{gfort + gcc version 5.3.0} or later, on SLC, and it is highly dependent on the compiler version.  Better portability coming soon.}
...
\end{verbatim}
The last argument tells MELA whether the event is a generator-level (false) event or a reconstructed event (true).  This affects how the output probabilities are normalized.
...
There is also a python interface for MELA, which works essentially the same way as the C++ interface.  For more information, use the python help function:
\begin{verbatim}
>>> import mela
>>> help(mela)
\end{verbatim}
Python by its nature is not always suitable for large-scale data processing, where speed is often important.  On the other hand, since text processing is simple in Python, the Python MELA interface is suitable for running quick jobs on LHE files.
%%%%%%%%%%%
