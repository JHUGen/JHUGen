...
\begin{itemize}
\item TEvtProb(double ebeam = 4000): Constructor with argument $\sqrt{s}$ in units of GeV.
\item void SetProcess(TVar::Process tmp): Set the process to be used in te calculation of the ME
\item void SetMatrixElement(TVar::MatrixElement tmp): Set the matrix element (Analytical, MCFM or JHUGen)
\item void SetProduction(TVar::Production tmp): Set the production (as enumerated in the TVar class)
\item void SetLeptonInterf(TVar::LeptonInterference tmp): Set whether lepton interference is included in decay MEs. By default, it is included for JHUGen decay MEs or MCFM $q\bar{q}$ background MEs. It is not included in the other MCFM MEs.
\item void ResetMCFM_EWKParameters(double ext_Gf, double ext_aemmz, double ext_mW, double ext_mZ, double ext_xW, int ext_ewscheme=3): Reset the MCFM EWK scheme and its parameters. See MCFM manual for the details on the EWK scheme.
\item void Set_LHAgrid(const char* path): One-time call to setup the PDF path in case NNPDF 3.0 is used.

\item double XsecCalc(TVar::Process proc,
...
\item double XsecCalcXJJ(TVar::Process proc, TVar::Production production, TLorentzVector p4[3],
TVar::VerbosityLevel verbosity,
...
\item double XsecCalcXJ(TVar::Process proc, TVar::Production production, TLorentzVector p4[2],TVar::VerbosityLevel verbosity): Call to the JHUGen $Hj$ ME is handled through tis routine.

\item double XsecCalc_VX(TVar::Process proc, TVar::Production production, vh_event_type \&vh_event,TVar::VerbosityLevel verbosity,double selfDHvvcoupl[SIZE_HVV_VBF][2]): Call to the JHUGen $VH$ ME is handled from this routine.

\item double XsecCalc_TTX(TVar::Process proc, TVar::Production production,tth_event_type \&tth_event,int topDecay,int topProcess,TVar::VerbosityLevel verbosity,double selfDHvvcoupl[SIZE_TTH][2]): Call to te $t\bar{t}H$ ME is handled through this routine.

\item void SetHiggsMass(double mass=125, float wHiggs=-1): This routine changes the Higgs mass and width used in the MCFM calculations.
