...
(version v7.1.0, release date Oct. 3, 2016, manual date Oct. 3, 2016) \\
\normalsize
\end{center}
...
\begin{verbatim}
  ...
  Higgs propagator and decay width:
    ...
    ReadPMZZ:          For ReweightDecay, read the decay probability distribution from a file
  Statistics options:
...
\item \verb|ReadPMZZ|: After each run using \verb|ReweightDecay|, the distribution is written to a file \verb|PMZZdistribution.out|.  This can be read in the next run if you specify \verb|ReadPMZZ=1| on the command line.  This only makes sense if the \verb|DecayModes|, couplings, etc. are the same as last time; otherwise you will read a distribution that no longer makes sense.
\end{itemize}
...
\noindent
In going from \verb|v7.0.0| to \verb|v7.1.0| (October 2, 2016), the updates are as follows:
