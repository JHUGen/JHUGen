...
(version v7.0.9, release date Nov. 26, 2017, manual date Nov. 26, 2017) \\
\normalsize
\end{center}
...
The latter was also introduced in Ref.~\cite{Chatrchyan:2012ufa}. The matrix element package (MELA) also depends on MCFM libraries for background parameterization which should be referenced~\cite{Campbell:2010ff} when used.
...
\noindent
In going from \verb|v7.0.2| to \verb|v7.0.9| (November 26, 2017), the updates are as follows:

\begin{itemize}
	\item MELA package interface is completely rewritten and streamlined, including new processes implemented
	\item Add the option for an intermediate $Z^\prime$ or $W^\prime$ in spin-0 resonance decay and in VBF and $VH$ production.  $V^\prime$ can have finite or infinite mass; in the second case, it is equivalent to the contact terms formulation~\cite{Gonzalez-Alonso:2014eva}
\end{itemize}
