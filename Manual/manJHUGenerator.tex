...
(version v7.2.0, release date September 21, 2018, manual date September 21, 2018) \\
\normalsize
\end{center}
...
\begin{itemize}
	...
	\end{itemize}
		\item $gg\to VH$:
	\begin{itemize}
		\item For $gg\to ZH$, use \texttt{Process=51} instead of \texttt{50}
		\item $ZH$ production with $a_2$ anomalous coupling, decay to $Z\to 2l,~H\to b\bar{b}$:
		\begin{verbatim}
		./JHUGen Collider=1 Process=51 VH_PC=gg HbbDK=1 DecayMode1=0 VegasNc2=100000 \\
		DataFile=ggZH2lbb ghz1=0,0 ghz2=1,0
	...
	\begin{itemize}
		...
		\item For the offshell VBF process, including up to two scalar resonances and interference with the SM background, see below.
	\end{itemize}
	\item $H+jj$:
	\begin{verbatim}
...
\begin{verbatim}
  ...
  Process configuration:
    ...
    VBFoffsh_run:      For VBF offshell production, set this to a number from 1-5
                       for each of the 5 jobs.  See below for more details.
...
\paragraph{\texorpdfstring{$gg\to VH$}{gg to VH}}
...
Processes 66 (signal), 67 (background), and 68 (signal+background+interference) provide an interface to a modified version of MCFM~\cite{Campbell:2010ff} to produce offshell VBF production $VV\to VV$, including all anomalous couplings.  For these processes, a second spin-zero resonance can be activated by setting \verb|MReso2| and \verb|GaReso2| to nonzero values.  The second resonance's couplings can be set to arbitrary values, similar to the first resonance's.

When producing unweighted VBF offshell events, jobs must be submitted in two steps, with five jobs per step.  Each set of five jobs can be run in parallel, but none of the jobs in the the second set can start until all five jobs of the first set finish.
...
\begin{enumerate}
	...
	\end{itemize}
	\item Event generation, which can only begin after all five previous jobs have ended:
	\begin{itemize}
...
\noindent
In going from \verb|v7.1.4| to \verb|v7.2.0| (September 21, 2018), the updates are as follows:
\begin{itemize}
\item Processes 66 (signal), 67 (background), and 68 (signal+background+interference) is added to generate offshell VBF events, including the continuum background and up to two scalar resonances.  The matrix elements are provided through an interface to MCFM, modified to include $HVV$ and $XVV$ anomalous couplings.
