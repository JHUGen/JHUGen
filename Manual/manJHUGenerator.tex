...
(version v7.5.4, release date August 25, 2023) \\
\normalsize
\end{center}
...
\section{Modified Zff couplings in Generation}
\subsection{JHUGen}
...
In production, we parameterize the modifications to the Zff couplings in production as shifts to the SM coupling of the Z-boson to up-type and down-type quarks. In decay, we allow the user to set the exact value of the Zff couplings to leptons and quarks. To utilize these features one must install the required package
\begin{verbatim}
cd MCFM-JHUGen
./Install
\end{verbatim}
Then the user can modify these couplings my manually editing the fortran code:
\begin{verbatim}
  src/User/mdata.f
\end{verbatim}
...
\section{Modified Zff couplings in MELA}

We support modified Zff couplings in certain MELA Processes. In MELA, all of the Zff couplings in decay are set directly in the interface. We only consider
anomalous Zqq couplings in production as shifts to the SM values for these couplings. The convention for the SM values is the same as in section VIII. for JHUGen
and MCFM matrix elements. If there is any anomalous Zff coupling set to a non-zero value in MELA all of the the other Zff couplings will be assumed to be 0
unless they are also explicitly set to some non-zero value.

\subsection{In JHUGen}

In JHUGen to set the Zff couplings manually in MELA:
...
\noindent 
In going from \verb|v7.5.2| to \verb|v7.5.4| (August 25, 2023) the updates are as follows:
\noindent
\begin{itemize}
