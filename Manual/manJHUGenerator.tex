...
\begin{verbatim}
   - Collider=x    (Collider: 1= $pp$, e.g LHC, 2= $p\bar{p}$, e.g. Tevatron, 0 = $e^+e^-$, e.g ILC), 
                   default value:1
   ...
   - Process=x     (gives the resonance spin 0,1 or 2, or 50=pp/ee->VH, 60=weakVBF, 61=pp->Hjj)
   - OffXVV=xyz    (only applies to Higgs production, x,y,z can be 1 or 0 indicating real off-shellness 
   					for Higgs, Z/W boson 1/2, resp.), default 011 if a parameter is not specified
   - Unweighted=x  (generates weighted or unweighted events), default value: ".true." 
   - ReadLHE="x"   (allows NLO production of spin-0 particles by reading in LHE file 
                   from NLO generator, e.g., POWHEG,   or allows generating spin-0 particle decay 
                   after generating VBF, H+JJ, or VH production with the JHU generator), 
                   default value: Ò.false." 
   - InputLHEFormat="x"   (LHE format is different for 1=POWHEG, 2=JHUGen), default value: 1 
\end{verbatim}   
...
real(8), public, parameter :: Rjet = 0.5d0                  ! jet deltaR, antikt algorithm \end{verbatim}
\subsubsection{ Spin-0 parameters }

\noindent
{\it N.B.  The parameters "ptjetcut" and "Rjet" only apply to Process=60,61 when using the "writeWeightedLHE=.true.".}
...
We now allow for $q^2$ dependent form factors as described in Ref.~\cite{Bolognesi:2012} and given in more detail in the equation below:
%\[
%g_i(q_1^2,q_2^2) = g_i^{\rm SM} + g_i' \times \frac{\Lambda_i^4}{(\Lambda_i^2 + |q_1^2|)(\Lambda_i^2 + |q_2^2|)}
%\]
\[
g_i(q_1^2,q_2^2) = g_i^{\rm SM} 
+ g_i' \frac{\Lambda_i^4}{(\Lambda_i^2 + |q_1^2|)(\Lambda_i^2 + |q_2^2|)}
+ g_i'' \frac{(|q_1^2|+|q_2^2|)}{\Lambda_i^2}
+ g_i''' \frac{(|q_1^2|+|q_2^2|)^2}{\Lambda_i^4}
+ g_i'''' \frac{|q_1^2| \, |q_2^2|}{\Lambda_i^4}
\]
\noindent
...
\item first version of VH generator
