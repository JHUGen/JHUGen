!      subroutine qq_ZZqq(p,msq,zzcoupl,wwcoupl,lambda_bsm,lambdaq,lambda)
...
!      endif
       ...
       ! Work the if-statement backward !--US
       if (Hbit .eq. czip) then
         doBO=.true.
       endif
       if (Bbit .eq. czip) then
         doHO=.true.
       endif
       if (doHO .and. doBO) then
         write(6,*) "Invalid doHO, doBO"
         doHO=.false.
         doBO=.false.
       endif
