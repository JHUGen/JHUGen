...
!      include 'first.f'
      logical, save :: firstZZHZZampF=.true.
!$omp threadprivate(firstZZHZZampF)
...
!$omp threadprivate(ZZ3456,ZZ1734,ZZ2856,ZZ1728)
      ...
      if (firstZZHZZampF) then
      firstZZHZZampF=.false.
...
C---setup propagators
      ...
      if ( (i1 .eq. 8) .and. (i7 .eq. 7) ) then !---test
        ZZHamp(jdu1,jdu2,h17,h28,h34,h56)=czip
        continue !---test
      elseif ( (i1 .eq. 7) .and. (i7 .eq. 8) ) then !---test
        ZZHamp(jdu1,jdu2,h17,h28,h34,h56)=czip
        continue !---test
      elseif ( (i1 .eq. 1) .and. (i7 .eq. 2) ) then !---test
        ZZHamp(jdu1,jdu2,h17,h28,h34,h56)=czip
        continue !---test
      elseif ( (i1 .eq. 2) .and. (i7 .eq. 1) ) then !---test
        ZZHamp(jdu1,jdu2,h17,h28,h34,h56)=czip
        continue !---test
      else !---test
!	  write(6,*) "i1, i2 = ",i1," ",i2
