...
c      j34 is flavour of 34 line
      ...
      include 'zcouple.f'
      include 'ewcharge.f'
      double precision p(mxpart,4),s134mmtsq,s234mmtsq,s124,s123,
     & s134,s234,s12,s34,s56,ampsq
      double complex spstrng0,spstrng1,
     ...
     & amp(2,2,2,2),propL(2,2),propR(2,2),prop
      ...
      p3(:)=dcmplx(p(i3,:))
      p4(:)=dcmplx(p(i4,:))
      p123(:)=dcmplx(p(i1,:)+p(i2,:)+p(i3,:))
      p124(:)=dcmplx(p(i1,:)+p(i2,:)+p(i4,:))
      p134(:)=dcmplx(p(i1,:)+p(i3,:)+p(i4,:))
      p234(:)=dcmplx(p(i2,:)+p(i3,:)+p(i4,:))
      p12(:)=dcmplx(p(i1,:)+p(i2,:))
      p34(:)=dcmplx(p(i3,:)+p(i4,:))
      p56(:)=dcmplx(p(i5,:)+p(i6,:))
      s12=dble(p12(4)**2-p12(1)**2-p12(2)**2-p12(3)**2)
      s34=dble(p34(4)**2-p34(1)**2-p34(2)**2-p34(3)**2)
      s56=dble(p56(4)**2-p56(1)**2-p56(2)**2-p56(3)**2)
      prop=s56/Dcmplx((s56-zmass**2),zmass*zwidth)
...
C---first index is flavour, second is helicity of 56 line
      ...
      s123=dble(p123(4)**2-p123(1)**2-p123(2)**2-p123(3)**2)
      s124=dble(p124(4)**2-p124(1)**2-p124(2)**2-p124(3)**2)
      s134mmtsq=s134-mt**2
      s234mmtsq=s234-mt**2
      h1=1
     ...
     &    spstrng1(e6p,p123,e1C)*s123**(-1) )
      ...
      do h56=1,2
      ampsq=ampsq
     & +dble(amp(h1,h2,h34,h56)*Dconjg(amp(h1,h2,h34,h56)))
      enddo
      enddo
      enddo
      enddo
