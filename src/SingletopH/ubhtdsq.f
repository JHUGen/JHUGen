      double precision function ubhtdsq(p1,p2,p3,p4,k5,p6,e5)
      implicit none
...
C     only enters through k5 and e5 in wave function
      ...
      include 'anomHiggs.f'
      double precision s25,s16,s345,mw
      integer p1,p2,p3,p4,k5,p6,e5
      double complex prW,prt,iza,izb,Ampw(2),Ampf(2),Ampt(2),zab2
...
c --   include anomalous Yukawa and EW couplings for the Higgs
      ...
      ubhtdsq=
     & +cdabs(Ampw(1)+Ampf(1)+Ampt(1))**2
     & +cdabs(Ampw(2)+Ampf(2)+Ampt(2))**2
