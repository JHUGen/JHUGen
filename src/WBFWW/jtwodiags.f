     ...
     & p1,p2,p3,p4,p5,p6,p7,p8,iswap
      double complex amp(2,2,2,2),propW34,propW56,
     & propW1347,propW1567,tmp
      double precision t3,t4,s34,s56,s1347,s1567,
...
c--- need to check corresponding result for (2,1,1,1)
     ...
     &    2.D0*za(p7,p5)*za(p8,p3)*zb(p1,p6)*zb(p2,p4)*cwmass2**(-1) )
      if (iswap .eq. 1) amp(1,2,1,1)=tmp
      if (iswap .eq. 2) amp(2,1,1,1)=tmp
     ...
     &     )
      if (iswap .eq. 1) amp(2,2,1,1)=tmp
      if (iswap .eq. 2) amp(1,1,1,1)=tmp
      enddo
