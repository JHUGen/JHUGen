...
c     u(-p1)+b(p2)->Z(p3,p4)+t(-> nu(p5) e+(p6) b(p7))+d(p6)
      ...
      double precision p(mxpart,4),p_dk(mxpart,4),q(mxpart,4),dot
      double precision sud,sbt,stz,suz,sbz,szd,sz,suda,ptDpep
      double complex amp_dk(2,2)
...
C      parameter(ph1=3,ph2=4)
      double precision sud,sbt,stz,suz,sbz,szd,sz,suda,mw
      double complex facuLl,facuRl,facdLl,nrp,nrm,cprop
      double complex prW,prt,prq,iza,izb,amp(2,2),iprZ
      prW(sz)=cone/dcmplx(sz-wmass**2,zip)
      prt(sz)=cone/dcmplx(sz-mt**2,zip)
      prq(sz)=cone/dcmplx(sz)
...
c--- Implementation of Baur-Zeppenfeld treatment of Z width
	     cprop=dcmplx(1d0/dsqrt((sz-zmass**2)**2+(zmass*zwidth)**2))
	     iprZ=dcmplx(sz-zmass**2)
      ...
      if (j .eq. 1) then
        pl=3
        pa=4
	    facuLl=dcmplx(Qu*q1)*iprZ+dcmplx(L(2)*le)*sz
	    facuRl=dcmplx(Qu*q1)*iprZ+dcmplx(R(2)*le)*sz
	    facdLl=dcmplx(Qd*q1)*iprZ+dcmplx(L(1)*le)*sz
      else
        pl=4
        pa=3
	    facuLl=dcmplx(Qu*q1)*iprZ+dcmplx(L(2)*re)*sz
	    facuRl=dcmplx(Qu*q1)*iprZ+dcmplx(R(2)*re)*sz
	    facdLl=dcmplx(Qd*q1)*iprZ+dcmplx(L(1)*re)*sz
      endif
